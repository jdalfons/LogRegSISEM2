...
LogisticRegression  <- R6Class("LogisticRegression",
                               public = list(
                                 ...
                                 softmax = function(z) {
                                   z = z - max(z)  # On soustrait la valeur maximale pour éviter des valeurs trop grandes
                                   exp_z = exp(z)
                                 ...
                                 # Prétraitement des données (conversion en matrices)
                                 #' @param X DataFrame
                                 preprocess_data = function(X) {
                                   ...
                                   if (any(is_numeric) && any(is_categorical)) {
                                     print("Données mixtes détectées : application de l'AFDM.")
                                     ...
                                     print("Données catégorielles détectées : application de l'ACM.")
                                     self$transformation_type <- "MCA"
                                     self$transformation_model <- MCA(X, graph = FALSE)
                                     processed_data <- as.data.frame(self$transformation_model$ind$coord)
                                   } else if (all(is_numeric)) {
                                     print("Données quantitatives détectées : normalisation.")
                                     ...
                                     stop("Impossible de déterminer le type de données.")
                                   }
                                 ...
                                 fit = function(X, y) {
                                   ...
                                   print(paste("Transformation utilisée dans fit après preprocess_data :", self$transformation_type))  # Vérification
                                   X <- as.matrix(X)
                                   m <- nrow(X)
                                   n <- ncol(X)
                                   ...
                                   print(paste("Classes encodées :", paste(unique(y), collapse = ", ")))
                                 ...
                                 preprocess_new_data = function(X_new) {
                                   if (is.null(self$transformation_type)) {
                                     stop("La transformation n'a pas été définie. Veuillez appeler la méthode `fit` avant `predict`.")
                                   }
                                   if (self$transformation_type == "FAMD") {
                                     print("Application de l'AFDM sur les nouvelles données.")
                                     processed_data <- predict(self$transformation_model, newdata = X_new)$coord
                                   } else if (self$transformation_type == "MCA") {
                                     print("Application de l'ACM sur les nouvelles données.")
                                     processed_data <- predict(self$transformation_model, newdata = X_new)$coord
                                   } else if (self$transformation_type == "scale") {
                                     print("Application de la normalisation sur les nouvelles données.")
                                     ...
                                     stop("Aucune transformation enregistrée pour ces données.")
                                   }
                                   return(processed_data)
                                 },
                                 ...
                                 predict = function(X) {
                                   ...
                                   print("Distribution des probabilités pour chaque classe :")
                                 ...
                                 var_importance = function() {
                                   ...
                                   # Retour du tableau pour un usage ultérieur
